...
def parse_financial_pdf_smart(tanshin_path, presentation_path=None):
    ...
    - 単なる数値の羅列や事実の転記は絶対に避けてください。
    - 現役トップクラスの証券アナリストとして、**「業績変動の背景にある構造的変化」「中長期的な企業価値への影響」「今後の株価カタリスト（変動要因）」**に踏み込んで深く洞察してください。
    - 読み手がひと目で判断できるよう、文頭に必ず「🟢ポジティブ：」「🔴ネガティブ：」「🟡中立：」のいずれかの絵文字ラベルをつけて、箇条書きで3〜4項目出力してください。
    - 【重要】例えばポジティブ要因なら「売上が〇〇増えた」ではなく、「〇〇の需要増が牽引しており、このトレンドは構造的で長期化する公算が大きい。利益率の改善も伴っており高く評価できる」のように、評価や推察を加えてください。
    - ネガティブ要因やリスクについても、単なる事実の指摘にとどまらず、それが一時的なものか、あるいは中長期的なダウンサイドリスクとなり得るのかをアナリスト目線で評価してください。
    ...
    {{
        ...
        "ai_analysis": {{
            "tab1_summary": "当期の業績について、プロのアナリストとして背景や構造変化に踏み込んだ深い洞察を、文頭に「🟢ポジティブ：」「🔴ネガティブ：」「🟡中立：」のラベルをつけた箇条書きで出力してください（数値の羅列は禁止）。【400文字程度】",
            "tab2_summary": "来期の見通し・経営戦略について、今後のカタリストやダウンサイドリスクを含め、投資家目線での深い洞察を、文頭に「🟢ポジティブ：」「🔴ネガティブ：」「🟡中立：」のラベルをつけた箇条書きで出力してください。【400文字程度】"
        }}
    }}
    """
    ...
    # 正規表現を用いてMarkdownのコードブロック記号を削除（構文エラーを完全に防止）
